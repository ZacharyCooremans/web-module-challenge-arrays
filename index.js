...
function getFlavorByIndex(originalFlavors, get){
    //console.log(originalFlavors[get]);
    return originalFlavors[get];
...
function removeFlavorByName(originalFlavors, remove){
   var index = originalFlavors.indexOf(remove);
   if (index > -1){
...
function filterByWord(originalFlavors, search){
    let typeArray = [];
    for(let i = 0; i < originalFlavors.length; i++){
        if(originalFlavors[i].includes(search)){
           typeArray.push(originalFlavors[i]);
        }
    }
    return typeArray;
}
